<template>
	<view class="content">
		<view class="topNavigationBar">
			<view :style="{marginTop:(titleBarTop+6)+'px'}">我的联系方式</view>
			<view class="backicon" @click="back" :style="{top:(titleBarTop+5)+'px'}"></view>
		</view>
		<view style="position: relative;">
			<input type="text" v-model="wxNumber" maxlength="20" focus>
			<view class="cancel" @click="cancel" v-if="wxNumber!=''"></view>
		</view>
	...
	export default {
		...
		methods: {
			...
			comfirm() {
				...
				uni.setStorageSync('wxNumber', this.wxNumber)
				let pages = getCurrentPages();
				let prevPage = pages[pages.length - 2];
				prevPage.$vm.wxNumber = this.wxNumber;
				this.back()
			},
			cancel() {
				this.wxNumber = ''
