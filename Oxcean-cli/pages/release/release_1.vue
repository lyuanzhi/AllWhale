	...
	<view class="background">
		...
		<view class="inputname">
			...
			<input type="text" style="margin-left: 10rpx; width: 70%; font-size: 28rpx;" @change="chan"
				placeholder="请输入物品名称（25字以内)" v-model="name">
		</view>
		<view class="inputdetail">
			<view class="font">补充信息</view>
			<textarea placeholder="说说物品的详细信息，例如型号、入手渠道、使用感受、转手原因" maxlength="200" v-model="detail"
		...
		<view class="label" @click="expand">
			<view class="font" style="margin-left: 50rpx;">分类/新旧/送货方式</view>
			<view style="color: #e20003; margin-left: 7rpx;">*</view>
		...
		<view class="labelbox" v-if="isExpend">
			<view class="labelbox-in">
				...
				<view class="scroll">
					<view v-for="(items,index) in labels.category" :key='index' @click="changeSelect(index,0)">
			...
			<view class="labelbox-in">
				...
				<view class="scroll">
					<view v-for="(items,index) in labels.condition" :key='index' @click="changeSelect(index,1)">
			...
			<view class="labelbox-in">
				...
				<view class="scroll">
					<view v-for="(items,index) in labels.delivery" :key='index' @click="changeSelect(index,2)">
		...
		<view class="line"></view>
		<view class='inputbox'>
			<view class="font" style="margin-left: 50rpx; white-space: nowrap;">价格 (£)</view>
			<view style="color: #e20003; margin-left: 7rpx;">*</view>
			<input type="digit" cursor-spacing="13"
				style="margin-left: 10%; width: 60%; text-align: right; font-size: 32rpx; color: #ff0000;"
				placeholder="请输入" v-model="currentprice" maxlength="10" @blur="checkCurrentprice">
		</view>
		<view class="line"></view>
		<view class='inputbox'>
			<view class="font" style="margin-left: 52rpx; white-space: nowrap;">原价 (£)</view>
			<input type="digit" cursor-spacing="13"
				style="margin-left: 12%; width: 60%; text-align: right; font-size: 32rpx; color: #ff0000;"
				placeholder="请输入" v-model="originalPrice" maxlength="10" @blur="checkOriginalprice">
		</view>
		<view class="line"></view>
		<view class='inputbox' @click='selectlocation'>
			<view class="font" style="margin-left: 50rpx; white-space: nowrap;">区域</view>
			<view style="color: #e20003; margin-left: 7rpx;">*</view>
			<view style="height: 40rpx;width: 60%; text-align: right; font-size: 32rpx; color: #686868;" v-model="area"
	...
	export default {
		data() {
			return {
				...
				isExpend: 1,
				labels: {
					category: ['美妆', '日用', '厨具', '电子', '其他'],
					condition: ['全新', '几乎全新', '略微使用', '日常折旧'],
					delivery: ['自取', '送货上门']
				},
				selectLabel: ['-1', '-1', '-1'],
				originalPrice: '',
		...
		methods: {
			...
			checkOriginalprice() {
				if (!(/(^[1-9]\d*(\.\d{1,2})?$)|(^0(\.\d{1,2})?$)/.test(this.originalPrice))) {
					this.originalPrice = ""
					uni.showToast({
						icon: "none",
						title: "请正确输入原价(最多2位小数)",
						duration: 2000
					})
				}
			},
					...
					success: (res) => {
						if (res.data.code == 200) {
							...
							let data = {
								...
								"name": this.name,
								"oldPrice": parseInt(this.originalPrice),
								"pickUpType": this.labels.delivery[this.selectLabel[2]],
